...
class Bradley:
    ...
    def __init__(self, chess_data: pd.DataFrame):
        ...
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write(f'White agent learn rate is: {self.W_rl_agent.settings.learn_rate}\n')
            self.print_statements_debug.write(f'White agent discount factor is: {self.W_rl_agent.settings.discount_factor}\n') 
            self.print_statements_debug.write(f'Black agent learn rate is: {self.B_rl_agent.settings.learn_rate}\n')
            self.print_statements_debug.write(f'Black agent discount factor is: {self.B_rl_agent.settings.discount_factor}\n')
    ...
    def recv_opp_move(self, chess_move: str) -> bool:                                                                                 
        ...
        """
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write(f"========== Hello from Bradley.recv_opp_move ==========\n\n")
            self.print_statements_debug.write("going to environ.load_chessboard\n")
        ...
        if self.environ.load_chessboard(chess_move):
            ...
            if PRINT_RESULTS_DEBUG:
                self.print_statements_debug.write("and we're back from environ.update_curr_state\n")
                self.print_statements_debug.write(f'Chessboard was successfully loaded with move: {chess_move}\n')
                self.print_statements_debug.write(f'Chessboard looks like this:\n\n')
                self.print_statements_debug.write(f'\n {self.environ.board}\n\n')
                self.print_statements_debug.write(f'Current turn is: {self.environ.get_curr_turn()}\n')
                self.print_statements_debug.write(f'Current turn index is: {self.environ.turn_index}\n')
                self.print_statements_debug.write("Bye from Bradley.recv_opp_move\n\n\n")

            return True
        else:
            self.error_log.write(f'Error: failed to load chessboard with move: {chess_move}\n')
            self.error_log.write("========== Bye from Bradley.recv_opp_move ==========\n\n\n")
            return False
    ...
    def rl_agent_selects_chess_move(self, rl_agent_color: str) -> dict[str]:
        ...
        """
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write(f"========== Hello from Bradley.rl_agent_selects_chess_move ==========\n\n")
            self.print_statements_debug.write("going to rl_agent.choose_action\n")
        ...
        if PRINT_RESULTS_DEBUG:
            ...
            self.print_statements_debug.write(f'Current turn is: {self.environ.get_curr_turn()}\n')
            self.print_statements_debug.write(f'Current turn index is: {self.environ.turn_index}\n')
            self.print_statements_debug.write("Bye from Bradley.rl_agent_selects_chess_move\n\n\n")

            return chess_move
        else:
            self.error_log.write(f'Error: failed to load chessboard with move: {chess_move}\n')
            self.error_log.write("========== Bye from Bradley.rl_agent_selects_chess_move ==========\n\n\n")
    ...
    def get_fen_str(self) -> str:
        ...
        """
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write(f"========== Hello from Bradley.get_fen_str ==========\n\n")

        try:
            if PRINT_RESULTS_DEBUG:
                self.print_statements_debug.write("going to environ.board.fen()\n")

            fen: str = self.environ.board.fen()

            if PRINT_RESULTS_DEBUG:
                self.print_statements_debug.write("and we're back from environ.board.fen()\n")
                self.print_statements_debug.write(f'FEN string is: {fen}\n')
                self.print_statements_debug.write("Bye from Bradley.get_fen_str\n\n\n")
            
            return fen
        except Exception as e:
            self.error_log.write(f'An error occurred: {e}\n')
            self.error_log.write("invalid board state or fen str was invalid\n")
            self.error_log.write("========== Bye from Bradley.get_fen_str ==========\n\n\n")
    ...
    def get_opp_agent_color(self, rl_agent_color: str) -> str:
        ...
        """
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write(f"========== Hello from Bradley.get_opp_agent_color ==========\n\n")

        if rl_agent_color == 'W':
            if PRINT_RESULTS_DEBUG:
                self.print_statements_debug.write(f'Opposing agent color is: B\n')
                self.print_statements_debug.write("Bye from Bradley.get_opp_agent_color\n\n\n")
            return 'B'
        else:
            if PRINT_RESULTS_DEBUG:
                self.print_statements_debug.write(f'Opposing agent color is: W\n')
                self.print_statements_debug.write("Bye from Bradley.get_opp_agent_color\n\n\n")
            return 'W'
    ...
    def get_curr_turn(self) -> str:
        ...
        """
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write(f"========== Hello from Bradley.get_curr_turn ==========\n\n")
            self.print_statements_debug.write("going to environ.get_curr_turn\n")
        
        curr_turn = self.environ.get_curr_turn()

        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write("and we're back from environ.get_curr_turn\n")
            self.print_statements_debug.write(f'Current turn is: {curr_turn}\n')
            self.print_statements_debug.write("Bye from Bradley.get_curr_turn\n\n\n")
    ...
    def get_legal_moves(self) -> list[str]:
        ...
        """
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write(f"========== Hello from Bradley.get_legal_moves ==========\n\n")
            self.print_statements_debug.write("going to environ.get_legal_moves\n")

        legal_moves = self.environ.get_legal_moves()

        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write("and we're back from environ.get_legal_moves\n")
            self.print_statements_debug.write(f'Legal moves are: {legal_moves}\n')
            self.print_statements_debug.write("========== Bye from Bradley.get_legal_moves ===========\n\n\n")
    ...
    def get_rl_agent_color(self) -> str: 
        ...
        """
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write(f"========== Hello from Bradley.get_rl_agent_color ==========\n\n")
            self.print_statements_debug.write(f'RL agent color is: {self.rl_agent.color}\n')
            self.print_statements_debug.write("========== Bye from Bradley.get_rl_agent_color ===========\n\n\n")
    ...
    def get_game_outcome(self) -> chess.Outcome or str:   
        ...
        """
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write(f"========== Hello from Bradley.get_game_outcome ==========\n\n")
            self.print_statements_debug.write("going to self.environ.board.outcome().result()\n")

        try:
            game_outcome = self.environ.board.outcome().result()

            if PRINT_RESULTS_DEBUG:
                self.print_statements_debug.write("and we're back from self.environ.board.outcome().result()\n")
                self.print_statements_debug.write(f'Game outcome is: {game_outcome}\n')
                self.print_statements_debug.write("========== Bye from Bradley.get_game_outcome ===========\n\n\n")

            return game_outcome
        except AttributeError as e:
            self.error_log.write(f'An error occurred: {e}\n')
            self.error_log.write("outcome not available, most likely game ended because turn_index was too high or player resigned\n")
            self.error_log.write("========== Bye from Bradley.get_game_outcome ===========\n\n\n")
    ...
    def get_game_termination_reason(self) -> str:
        ...
        """
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write(f"========== Hello from Bradley.get_game_termination_reason ==========\n\n")
            self.print_statements_debug.write("going to self.environ.board.outcome().termination\n")

        try:
            termination_reason = str(self.environ.board.outcome().termination)

            if PRINT_RESULTS_DEBUG:
                self.print_statements_debug.write("and we're back from self.environ.board.outcome().termination\n")
                self.print_statements_debug.write(f'Termination reason is: {termination_reason}\n')
                self.print_statements_debug.write("========== Bye from Bradley.get_game_termination_reason ===========\n\n\n")

            return termination_reason
        except AttributeError as e:
            self.error_log.write(f'An error occurred: {e}\n')
            self.error_log.write("termination reason not available, most likely game ended because turn_index was too high or player resigned")
            self.error_log.write("========== Bye from Bradley.get_game_termination_reason ===========\n\n\n")
    ...
    def train_rl_agents(self) -> None:
        ...
        """ 
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write(f"========== Hello from Bradley.train_rl_agents ==========\n\n")

        W_curr_Qval: int = self.settings.initial_q_val
        B_curr_Qval: int = self.settings.initial_q_val

        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write(f'White agent initial Q value is: {W_curr_Qval}\n')
            self.print_statements_debug.write(f'Black agent initial Q value is: {B_curr_Qval}\n')
            self.print_statements_debug.write("entering main for loop for training\n")
        ...
        for game_num_str in self.chess_data.index:
            ...
            if PRINT_RESULTS_DEBUG:
                self.print_statements_debug.write("entering self.environ.get_curr_state()\n")

            curr_state: dict[str, str, list[str]] = self.environ.get_curr_state()

            if PRINT_RESULTS_DEBUG:
                self.print_statements_debug.write("and we're back from self.environ.get_curr_state()\n")
                self.print_statements_debug.write(f'Current state is: {curr_state}\n')
                self.print_statements_debug.write("now entering loop for ONE game\n")
            ...
            while curr_state['turn_index'] < num_chess_moves_curr_training_game:
                ...
                if PRINT_RESULTS_DEBUG:
                    self.print_statements_debug.write(f'White agent picks move from state: {curr_state}\n')
                    self.print_statements_debug.write("going to self.rl_agent_PICKS_move_training_mode\n")

                W_chess_move = self.rl_agent_PICKS_move_training_mode(curr_state, self.W_rl_agent.color, game_num_str)

                if PRINT_RESULTS_DEBUG:
                    self.print_statements_debug.write("and we're back from self.rl_agent_PICKS_move_training_mode\n")
                    self.print_statements_debug.write(f'White agent picked move: {W_chess_move}\n')
                    self.print_statements_debug.write(f'on turn: {curr_state["turn_index"]}\n')
                ...
                if PRINT_RESULTS_DEBUG:
                    self.print_statements_debug.write(f'White agent assigns points to Q table for move: {W_chess_move}\n')
                    self.print_statements_debug.write("going to self.assign_points_to_Q_table_training_mode\n")

                self.assign_points_to_Q_table_training_mode(W_chess_move, curr_turn, W_curr_Qval, self.W_rl_agent.color)

                if PRINT_RESULTS_DEBUG:
                    self.print_statements_debug.write("and we're back from self.assign_points_to_Q_table_training_mode\n")
                    self.print_statements_debug.write(f'White agent assigned points to Q table for move: {W_chess_move}\n')
                    self.print_statements_debug.write("now going to rl_agent_PLAYS_move_training_mode\n")
                ...
                if PRINT_RESULTS_DEBUG:
                    self.print_statements_debug.write("and we're back from rl_agent_PLAYS_move_training_mode\n")
                ...
                if curr_state['turn_index'] >= num_chess_moves_curr_training_game:
                    if PRINT_RESULTS_DEBUG:
                        self.print_statements_debug.write(f'Game ended on White turn\n')
                        self.print_statements_debug.write(f'curr_state["turn_index"] is: {curr_state["turn_index"]}\n')
                        self.print_statements_debug.write(f'num_chess_moves_curr_training_game is: {num_chess_moves_curr_training_game}\n')
                    break
                else:
                    if PRINT_RESULTS_DEBUG:
                ...
                if PRINT_RESULTS_DEBUG:
                    self.print_statements_debug.write("going to assign_points_to_Q_table_training_mode\n")
                ...
                # take action a, observe r, s', and load chessboard
                if PRINT_RESULTS_DEBUG:
                    self.print_statements_debug.write("going to rl_agent_PLAYS_move_training_mode\n")

                B_reward = self.rl_agent_PLAYS_move_training_mode(B_chess_move)

                if PRINT_RESULTS_DEBUG:
                    self.print_statements_debug.write("and we're back from rl_agent_PLAYS_move_training_mode\n")
                    self.print_statements_debug.write(f'B reward is: {B_reward} for playing move: {B_chess_move}\n')
                    self.print_statements_debug.write("going to self.environ.get_curr_state\n")
                ...
                if curr_state['turn_index'] >= num_chess_moves_curr_training_game:
                    ...
                    if PRINT_RESULTS_DEBUG:
                        self.print_statements_debug.write(f'Game ended on Blacks turn\n')
                        self.print_statements_debug.write(f'curr_state["turn_index"] is: {curr_state["turn_index"]}\n')
                        self.print_statements_debug.write(f'num_chess_moves_curr_training_game is: {num_chess_moves_curr_training_game}\n')
                    ...
                    if PRINT_RESULTS_DEBUG:
                        self.print_statements_debug.write("going to find_estimated_Q_value")

                    B_est_Qval: int = self.find_estimated_Q_value()

                    if PRINT_RESULTS_DEBUG:
                        self.print_statements_debug.write("and we're back from find_estimated_Q_value")
                        self.print_statements_debug.write(f'estimated b q val is: {B_est_Qval}\n')
                        self.print_statements_debug.write("going to SARSA calculations\n")
                ...
                if PRINT_RESULTS_DEBUG:
                    self.print_statements_debug.write("and we're back from SARSA calculations methods, find_next_Qval\n")
                    self.print_statements_debug.write("SARSA calc was successful\n")
                    self.print_statements_debug.write(f'W next Q val is: {W_next_Qval}\n')
                    self.print_statements_debug.write(f'B next Q val is: {B_next_Qval}\n')
                    self.print_statements_debug.write("going to self.environ.get_curr_state\n")
            ...
            if PRINT_RESULTS_DEBUG:
                self.print_statements_debug.write(f'Game {game_num_str} is over.\n')
                self.print_statements_debug.write(f'\nThe Chessboard looks like this:\n')
                self.print_statements_debug.write(f'\n {self.environ.board}\n\n')
                self.print_statements_debug.write(f'Game result is: {self.get_game_outcome()}\n')
                self.print_statements_debug.write(f'The game ended because of: {self.get_game_termination_reason()}\n')
                self.print_statements_debug.write("going to self.reset_environ\n")
            self.reset_environ()

            if PRINT_RESULTS_DEBUG:
                self.print_statements_debug.write("and we're back from self.reset_environ\n")
                self.print_statements_debug.write("going to next game in training session\n")
        ...
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write(f'White agent is trained: {self.W_rl_agent.is_trained}\n')
            self.print_statements_debug.write(f'Black agent is trained: {self.B_rl_agent.is_trained}\n')
            self.print_statements_debug.write("========== Bye from Bradley.train_rl_agents ===========\n\n\n")
    ...
    def rl_agent_PICKS_move_training_mode(self, curr_state: dict[str, str, list[str]], rl_agent_color: str, game_num_str: str = 'Game 1') -> str:
        ...
        """
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write(f"========== Hello from Bradley.rl_agent_PICKS_move_training_mode ==========\n\n")
            self.print_statements_debug.write(f'Current state is: {curr_state}\n')
            self.print_statements_debug.write(f'RL agent color is: {rl_agent_color}\n')
            self.print_statements_debug.write(f'Game number is: {game_num_str}\n')
            self.print_statements_debug.write("going to rl_agent.choose_action\n")
        ...
        if rl_agent_color == 'W':
            ...
            if PRINT_RESULTS_DEBUG:
                self.print_statements_debug.write("and we're back from rl_agent.choose_action\n")
                self.print_statements_debug.write(f'White agent picked move: {curr_action["chess_move_str"]}\n')
        else:
            curr_action: dict[str] = self.B_rl_agent.choose_action(curr_state, game_num_str)

            if PRINT_RESULTS_DEBUG:
                self.print_statements_debug.write("and we're back from rl_agent.choose_action\n")
                self.print_statements_debug.write(f'Black agent picked move: {curr_action["chess_move_str"]}\n')
        
        chess_move: str = curr_action['chess_move_str']

        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write("========== Bye from Bradley.rl_agent_PICKS_move_training_mode ===========\n\n\n")
    ...
    def assign_points_to_Q_table_training_mode(self, chess_move: str, curr_turn: str, curr_Qval: int, rl_agent_color: str) -> None:
        ...
        """
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write(f"========== Hello from Bradley.assign_points_to_Q_table_training_mode ==========\n\n")
            self.print_statements_debug.write(f'Chess move is: {chess_move}\n')
            self.print_statements_debug.write(f'Current turn is: {curr_turn}\n')
            self.print_statements_debug.write(f'Current Q value is: {curr_Qval}\n')
            self.print_statements_debug.write(f'RL agent color is: {rl_agent_color}\n')
            self.print_statements_debug.write("and we're going to rl_agent.change_Q_table_pts\n")
        ...
        if rl_agent_color == 'W':
            try:
                ...
                if PRINT_RESULTS_DEBUG:
                    self.print_statements_debug.write("and we're back from rl_agent.change_Q_table_pts\n")
                    self.print_statements_debug.write(f'White agent changed Q table points for move: {chess_move}\n')

            except KeyError: 
                # chess move is not represented in the Q table, update Q table and try again.
                if PRINT_RESULTS_DEBUG:
                    self.print_statements_debug.write(f'Chess move is not represented in the White Q table, updating Q table and trying again...\n')
                    self.print_statements_debug.write("going to self.W_rl_agent.update_Q_table\n")

                self.W_rl_agent.update_Q_table([chess_move])

                if PRINT_RESULTS_DEBUG:
                    self.print_statements_debug.write("and we're back from self.W_rl_agent.update_Q_table\n")
                    self.print_statements_debug.write("going to self.W_rl_agent.change_Q_table_pts\n")

                self.W_rl_agent.change_Q_table_pts(chess_move, curr_turn, curr_Qval)

                if PRINT_RESULTS_DEBUG:
                    self.print_statements_debug.write("and we're back from self.W_rl_agent.change_Q_table_pts\n")
                    self.print_statements_debug.write(f'White agent changed Q table points for move: {chess_move}\n')
        else:
            try:
        ...
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write("========== Bye from Bradley.assign_points_to_Q_table_training_mode ===========\n\n\n")
    ...
    def rl_agent_PLAYS_move_training_mode(self, chess_move: str) -> int:
        ...
        """
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write(f"========== Hello from Bradley.rl_agent_PLAYS_move_training_mode ==========\n\n")
            self.print_statements_debug.write(f'Chess move is: {chess_move}\n')
            self.print_statements_debug.write("going to self.environ.load_chessboard\n")

        self.environ.load_chessboard(chess_move)

        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write("and we're back from self.environ.load_chessboard\n")
            self.print_statements_debug.write("going to self.environ.update_curr_state\n")

        self.environ.update_curr_state()

        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write("and we're back from self.environ.update_curr_state\n")
            self.print_statements_debug.write("going to self.analyze_board_state\n")
        ...
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write("and we're back from self.analyze_board_state\n")
            self.print_statements_debug.write(f'Analysis results are: {analysis_results}\n')
        ...
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write(f'Reward for move is: {reward}\n')
            self.print_statements_debug.write("========== Bye from Bradley.rl_agent_PLAYS_move_training_mode ===========\n\n\n")
    ...
    def find_estimated_Q_value(self) -> int:
        ...
        """
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write(f"========== Hello from Bradley.find_estimated_Q_value ==========\n\n")
            self.print_statements_debug.write("going to self.analyze_board_state\n")
        ...
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write("and we're back from self.analyze_board_state\n")
            self.print_statements_debug.write(f'Analysis results are: {analysis_results}\n')
            self.print_statements_debug.write("going to self.environ.load_chessboard_for_Q_est\n")
        ...
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write("and we're back from environ.load_chessboard_for_Q_est\n")
            self.print_statements_debug.write("going to analyze_board_state")
        ...
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write("and we're back from analyze_board_state")
        ...
        # playing a move.
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write("going to self.environ.pop_chessboard\n")

        self.environ.pop_chessboard()
        
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write("and we're back from self.environ.pop_chessboard\n")
            self.print_statements_debug.write(f'estimated q val is: {est_Qval}\n')
            self.print_statements_debug.write("========== Bye from Bradley.find_estimated_Q_value ===========\n\n\n")
    ...
    def find_next_Qval(self, curr_Qval: int, learn_rate: float, reward: int, discount_factor: float, est_Qval: int) -> int:
        ...
        """
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write(f"========== Hello from Bradley.find_next_Qval ==========\n\n")
            self.print_statements_debug.write(f'Current Q value is: {curr_Qval}\n')
            self.print_statements_debug.write(f'Learning rate is: {learn_rate}\n')
            self.print_statements_debug.write(f'Reward is: {reward}\n')
            self.print_statements_debug.write(f'Discount factor is: {discount_factor}\n')
            self.print_statements_debug.write(f'Estimated Q value is: {est_Qval}\n')

        next_Qval = curr_Qval + learn_rate * (reward + ((discount_factor * est_Qval) - curr_Qval))

        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write(f'Next Q value is: {next_Qval}\n')
            self.print_statements_debug.write("========== Bye from Bradley.find_next_Qval ===========\n\n\n")
        return next_Qval
    ...
    def analyze_board_state(self, board: chess.Board, is_for_est_Qval_analysis: bool = True) -> dict:
        ...
        """
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write(f"========== Hello from Bradley.analyze_board_state ==========\n\n")
            self.print_statements_debug.write(f'Board is: {board}\n')
            self.print_statements_debug.write(f'is_for_est_Qval_analysis is: {is_for_est_Qval_analysis}\n')
        ...
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write(f'Analysis result is: {analysis_result}\n')
        ...
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write(f'Normalized mate score is: {mate_score}\n')
            self.print_statements_debug.write(f'Normalized centipawn score is: {centipawn_score}\n')
        ...
        if is_for_est_Qval_analysis:
            ...
            if PRINT_RESULTS_DEBUG:
                self.print_statements_debug.write(f'Anticipated next move is: {anticipated_next_move}\n')
                self.print_statements_debug.write("========== Bye from Bradley.analyze_board_state ===========\n\n\n")
    ...
    def get_reward(self, chess_move_str: str) -> int:                                     
        ...
        """
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write(f"========== Hello from Bradley.get_reward ==========\n\n")
            self.print_statements_debug.write(f'Chess move is: {chess_move_str}\n')
        ...
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write(f'Total reward is: {total_reward}\n')
            self.print_statements_debug.write("========== Bye from Bradley.get_reward ===========\n\n\n")
    ...
    def reset_environ(self) -> None:
        ...
        """
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write(f"========== Hello from Bradley.reset_environ ==========\n\n")
            self.print_statements_debug.write("going to self.environ.reset_environ\n")
        
        self.environ.reset_environ()
        
        if PRINT_RESULTS_DEBUG:
            self.print_statements_debug.write("and we're back from self.environ.reset_environ\n")
            self.print_statements_debug.write("Environment has been reset\n")
            self.print_statements_debug.write("========== Bye from Bradley.reset_environ ===========\n\n\n")
